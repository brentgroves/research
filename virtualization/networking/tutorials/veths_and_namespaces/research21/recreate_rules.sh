...
iptables -A FORWARD -i enp0s25 -o veth0 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

iptables -D FORWARD -i veth0 -o enp0s25 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT
iptables -A FORWARD -i veth0 -o enp0s25 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

iptables -t nat -D PREROUTING -i enp0s25 -p tcp -m tcp --dport 8080 -j DNAT --to-destination 192.168.0.2
iptables -t nat -A PREROUTING -i enp0s25 -p tcp --dport 8080 -j DNAT --to-destination 192.168.0.2

iptables -t nat -D PREROUTING -i enp0s25 -p tcp -m tcp --dport 6000 -j DNAT --to-destination 192.168.0.2:8080
iptables -t nat -A PREROUTING -i enp0s25 -p tcp --dport 6000 -j DNAT --to-destination 192.168.0.2:8080
